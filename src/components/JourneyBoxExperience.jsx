...
/* ─────────────────────────────────────────────────────────────────────────────
   STAGE DATA (4 Checkpoints & Story Cards)
───────────────────────────────────────────────────────────────────────────── */
const STAGES = [
  {
    num: '01',
  ...
  },
  {
    ...
    desc: 'Companies gain access to talent that has already worked in professional environments and understands real workflows.',
    side: 'left',
    activateAt: 0.74,
    icon: (
      <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor">
...
/* ─────────────────────────────────────────────────────────────────────────────
   CHECKPOINT NODE POSITIONS (Percentage coordinates across 100vw x 100vh scene)
───────────────────────────────────────────────────────────────────────────── */
const NODES_POS = [
  { x: 22, y: 26 },
  { x: 74, y: 42 },
...
/* EXACTLY ONE continuous S-curve SVG path passing through every node */
const ZIG_PATH = [
  'M 22 18',
  'C 22 22, 22 24, 22 26',
  'C 22 36, 74 32, 74 42',
  'C 74 52, 24 52, 24 62',
  'C 24 72, 76 68, 76 78',
  'C 76 84, 90 84, 100 86',
].join(' ');

// Normalized path progression targets where stroke reaches each node
const NODE_PATH_TARGETS = [0.10, 0.36, 0.60, 0.84];
...
export default function JourneyBoxExperience() {
  ...
  useGSAP(() => {
    ...
    function tick() {
      ...
      /* 1. Full-screen box canvas frame animation */
      drawFrameAtProgress(p);

      /* 2. Single continuous SVG path draw */
      const pathP = clamp01(remap(p, 0.08, 0.86, 0, 1));
      pathEl.style.strokeDashoffset = (pathLen * (1 - pathP)) + 'px';

      /* 3. Headline overlay fade out */
      const head = headlineRef.current;
      if (head) {
        const ho = clamp01(remap(p, 0.08, 0.20, 1, 0));
        head.style.opacity = ho;
        head.style.transform = 'translateY(' + ((1 - ho) * -24) + 'px)';
      }

      /* 4. Checkpoint Activation, Single Card Handoff, and Path Node Pulse */
      nodeRefs.current.forEach((node, i) => {
        if (!node) return;

        const windowStart = STAGES[i].activateAt;
        const windowEnd = i < 3 ? STAGES[i + 1].activateAt : 0.86;
  ...
  return (
    ...
    >
      {/* ── STICKY VIEWPORT (100vw x 100vh) — 9:16 FULL-SCREEN PORTRAIT SCENE ─────────────── */}
      <div className="jbe-sticky">
        ...
        {/* 9:16 PORTRAIT VISUAL CONTAINER — HERO PRODUCT CANVAS */}
        <div className="jbe-full-canvas-wrap">
          <div className="jbe-portrait-box-container">
            <canvas ref={canvasRef} className="jbe-canvas" aria-hidden="true" />
          </div>
          <ParticleCanvas />
        </div>

        {/* OVERLAY: EDITORIAL HEADLINE BLOCK */}
        ...
        <div className="jbe-overlay-scene" aria-hidden="true">
          ...
          {/* Checkpoints — Circular nodes sitting directly ON the path */}
          {STAGES.map((stage, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Floating Story Card — Active Handoff Animation */}
